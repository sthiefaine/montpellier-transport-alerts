...
export async function GET(request: Request) {
  try {
    ...
    if (isNaN(days) || days <= 0 || days > 92) {
      // Open-Meteo limite à ~3 mois d'historique
    ...
    console.log(
      `Récupération des données météo du ${startDateStr} au ${endDateStr}`
    );
    ...
    for (const location of LOCATIONS) {
      ...
      console.log(
        `${weatherData.length} points de données récupérés pour ${location.id}`
      );
...
async function saveWeatherData(weatherData: any[]): Promise<number> {
  ...
  // Filtrer les données incomplètes avant le traitement
  const validWeatherData = weatherData.filter((data) => {
    ...
    if (!isValid) {
      console.warn(`Donnée météo incomplète ignorée: ${JSON.stringify(data)}`);
      skippedCount++;
    }
    return isValid;
  });
  ...
  console.log(
    `Traitement de ${validWeatherData.length} enregistrements valides (${skippedCount} ignorés)`
  );

  // Traiter par lots pour éviter les problèmes de performance
  const batchSize = 100;

  for (let i = 0; i < validWeatherData.length; i += batchSize) {
    const batch = validWeatherData.slice(i, i + batchSize);
    ...
    const operations = batch.map((data) => {
      return prisma.weatherData.upsert({
        ...
        update: {
          ...
          weatherType: data.weatherType || "Unknown",
    ...
    try {
      ...
      console.error(
        `Erreur pendant l'insertion du lot ${i}-${i + batch.length}:`,
        error
      );
      console.error(`Premier élément du lot problématique:`, batch[0]);
      throw error; // Relancer l'erreur après journalisation détaillée
    }
  }
