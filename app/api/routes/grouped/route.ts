...
export async function GET() {
  try {
    ...
    try {
      // Dans le schéma Prisma, les champs sont mappés avec des noms différents dans la BDD
      headsignsData = await prisma.$queryRaw<HeadsignQueryResult[]>`
        ...
        FROM 
          trips
        WHERE 
          trip_headsign IS NOT NULL
        GROUP BY 
          route_id, direction_id, trip_headsign
        ORDER BY 
          route_id, direction_id, COUNT(*) DESC
      `;
    ...
    headsignsData.forEach((record) => {
      ...
      const directionId =
        record.directionId === null ? 0 : Number(record.directionId);
      ...
      if (!directionMap.has(directionId)) {
        ...
        console.log(
          `Route ${routeKey}, direction ${directionId} → Headsign: "${record.headsign}"`
        );
      }
    });
    ...
    const enrichedRoutes: EnrichedRoute[] = Object.entries(routeGroups).map(
      ([shortName, group]) => {
        ...
        if (group.length === 1) {
          ...
          const routeHeadsigns =
            routeDirectionHeadsigns.get(route.id) || new Map<number, string>();
          console.log(
            `Headsigns for route ${route.id}:`,
            Object.fromEntries(routeHeadsigns)
          );

          // Extraction intelligente des destinations à partir du nom de la ligne
          let destinationNames = extractDestinationsFromRouteName(
            route.longName
          );

          // Utiliser des noms de destination significatifs ou des valeurs par défaut
          const headsign0 =
            routeHeadsigns.get(0) ||
            destinationNames.destination ||
            "Direction Aller";
          const headsign1 =
            routeHeadsigns.get(1) ||
            destinationNames.origin ||
            "Direction Retour";
        ...
        const alternativeIds =
          mainId === bestRoute.id
            ? otherIds
            : [bestRoute.id, ...otherIds.filter((id) => id !== mainId)];

        // Récupérer les headsigns pour les directions
        const routeHeadsigns =
          routeDirectionHeadsigns.get(bestRoute.id) ||
          new Map<number, string>();
        console.log(
          `Headsigns for best route ${bestRoute.id}:`,
          Object.fromEntries(routeHeadsigns)
        );
